...
class ConfMat(ConfusionMatrix):
    def __init__(self, num_classes: int, *args, **kwargs):
        self.component_names = [f"cf_{i}_{j}" for i in range(num_classes) for j in range(num_classes)]
    ...
    def compute(self) -> Tensor:
        ...
        names = [f"cf_{i}_{j}" for i in range(self.num_classes) for j in range(self.num_classes)]
...
def get_metric_titles_components_mapping(metrics):
    m_dict = {
        metric_name:
        metric.component_names if hasattr(metric, "component_names") else [metric_name]
